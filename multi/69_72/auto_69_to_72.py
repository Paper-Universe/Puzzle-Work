...
total = 3836922767331586736128 # Max Range in Decimal. Max Range = (end range - start)
...
while exists('Found.txt') == False:
    if exists('Found.txt') == True:
        exit()
    rand = random.randint(0x10000000, 0xffffffff) # currently 7 0's and f's or 40 bit
    if rand in range(536870912, 1073741823): # remember random.randint is only generating a number between an 8 digit hex the full range gets added in later
        continue
    hexify = hex(rand)
    stripped = hexify[2:]
